...
def polemix (f, fc, a, g):
    # Pole mixing transfer function
    pm = 0
    for ia in range(len(a)):
        pm += a[ia] * g[ia] * Hn (f, fc, ia)
    return pm
...
#    plt.rcParams["figure.figsize"] = [7.5, 4.5]  # plot size in inches
    ...
    corner = 1000
    page = [[[], [], [], [], [], 'b', '']] # each entry is list of coefs, list of labels, list of corners, list of polemix results, list of colors, plots, path
    for a in argv[1:]:
        if a[0] == '-':
            if len(a) < 2:
                usage()
        ...
        err = 0.001  # Uncorrelated error size
        for i in range(10):
            cc = [gauss(1, err) for ii in range(5)]
            page[-1][0].append([float (i) for i in re.split (", *", g1)])
            page[-1][1].append(g2+" "+" ".join(["{:5.3f}".format(cci) for cci in cc]))
            page[-1][2].append(corner)
            page[-1][3].append(polemix (f, corner, page[-1][0][-1], cc))
            page[-1][4].append("tab:orange")
        err = 0.0025  # Correlated error size
        for i in range(10):
            g =gauss(1, err)
            cc = [gauss(g, err/4) for ii in range(5)]
            page[-1][0].append([float (i) for i in re.split (", *", g1)])
            page[-1][1].append(g2+" "+" ".join(["{:5.3f}".format(cci) for cci in cc]))
            page[-1][2].append(corner)
            for i in range (1, 5):
                cc[i] = cc[i-1]*cc[i]
            page[-1][3].append(polemix (f, corner, page[-1][0][-1], cc))
            page[-1][4].append("tab:green")
        cc = [1 for ii in range(5)]
        page[-1][0].append([float (i) for i in re.split (", *", g1)])
        page[-1][1].append(g2)
        page[-1][2].append(corner)
        page[-1][3].append(polemix (f, corner, page[-1][0][-1], cc))
        page[-1][4].append("tab:blue")
    ...
    for j in range (len(page)):
        ...
        if page[j][5] == 'b':
            plt.subplot(211)
        if page[j][5] == 'a' or page[j][5] == 'b':
            for i in range(len(page[j][3])):
                plt.plot(f, 20*np.log10(abs(page[j][3][i])), label=page[j][1][i], c=page[j][4][i])
            plt.xscale('log')
        ...
        if page[j][5] == 'b':
            plt.subplot(212)
        if page[j][5] == 'p' or page[j][5] == 'b':
            for i in range(len(page[j][3])):
                plt.plot(f, 180./np.pi*np.angle(page[j][3][i]), label=page[j][1][i], c=page[j][4][i])
            plt.xscale('log')
